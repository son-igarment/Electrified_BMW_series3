%% Electrified BMW Series 3 - Brushless Motor Characteristics
% Owner: Phạm Lê Ngọc Sơn
...
% r: rapporto tra le induttanze di asse d ed asse q (eccentricit dell'ellisse)
r=Ld/Lq
r1=1-1/r;
% eseguo i calcoli per ciascun valore di velocit
n= [10:50:12000];
w=p*2*pi*n/60;
%b: parametro rapporto (semiasse maggiore ellisse/raggio cerchio)
% i: indice del vettore correnti
% k: indice del vettore velocit
for  i=1:length(In)


%
%
%%% tratto a bassa velocit:  da w=0 ad w=wB
%
...
%velocit wB fino a cui si ha la coppia massima
% calcolo la velocit base a cui avviene l'intersezione tra MTC e cerchio
...
b(i,:)=Vo/Lq./In(i)./w;
%% trovo l'indice del vettore w delle velocit per cui w<=wB
iwB=find(w<=wB(i));
iiwB=length(iwB);
...
%deltaBC: angolo delta (tra tensione e fem) nel tratto di velocit media
...
% grafici in funzione della velocit
figure
subplot(3,1,1)
...
grid
xlabel ('speed [rpm]')
%%% tratto a velocit intermedia da wB a wC, 
...
%l'elemento n.5 quello con la corrente nominale%%%
...
%%%disegno la mappa di efficienza sul diagramma coppia-velocit in pu
...
% ad alta velocit non detto che sia la stessa curva a bassa
for i=1:length(w)
